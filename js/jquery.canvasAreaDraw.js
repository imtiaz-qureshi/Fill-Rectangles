(function( $ ){
  ...
  var init = function(index, input, options) {
    ...
    $reset = $('<button type="button" class="btn btn-danger"><i class="icon-trash"></i>Clear Polygon</button>');
    $canvas = $('<canvas>');
    ctx = $canvas[0].getContext('2d');

    image = new Image();
    resize = function() {
      $canvas.attr('height', "500").attr('width', $("#canvas-container").css("width"));
      draw();
    };
    $(image).load(resize);
    ...
    $(document).ready( function() {
      $(input).after('<div id="canvas-container" style="position:relative;"></div><br>');
      $("#canvas-container").append($canvas);
      $("#clearButtonContainer").html($reset);
      setTimeout(function(){
        //alert($("#canvas-container").children("canvas").attr("width"));
        ...
        $("#canvas-container").children("canvas").style("position", "absolute");
